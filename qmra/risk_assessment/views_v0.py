...
from qmra.scenario.views import ExposureForm
...
from qmra.source.views import InflowFormSet, WaterSourceForm
from qmra.treatment.models import Treatment, Pathogen, LogRemoval
from qmra.source.models import WaterSource
from qmra.treatment.views import TreatmentForm, LogRemovalFormSet
from crispy_forms.layout import Layout, Fieldset, Field, Div


# TODO:
# select categories of [source, treatment, exposure, pathogen]
# new data model
# ra form and views
# POST fieldset for plots (treatment, inflows)
# create and serve json static files for
#   - default pathogens
#   - default inflows
#   - default ...
...
class RAForm(forms.ModelForm):
    def __init__(self, user, *args, **kwargs):
        ...
        self.helper.field_class="mygrid"
        self.helper.form_tag = False
        self.helper.disable_csrf = True
        self.helper.layout = Layout(
            "name", "description", "source",
            Div("treatment", field_id="ra-treatment-boxes"),
            "exposure"
        )
    ...
    class Meta:
        ...
        widgets = {
            "source": forms.Select(attrs={"empty_label": None}),
...
def inline_risk_assessment(request, risk_assessment_id = None):
    if request.method == "POST":
        inflow_form = InflowFormSet(request.POST)
        treatment_form = LogRemovalFormSet(request.POST)
        exposure_form = ExposureForm(request.POST)
        any_error = not all(f.is_valid() for f in (inflow_form, treatment_form, exposure_form))
        if any_error:
            return render(request, "assessment-configurator.html",
                          dict(inflow_form=inflow_form,
                               treatment_form=treatment_form,
                               exposure_form=exposure_form))
        return HttpResponseRedirect(reverse("assessment"))
    ra_form = RAForm(request.user, prefix="ra")
    ra_form.helper.form_tag = False
    source_form = WaterSourceForm()
    inflow_form = InflowFormSet(queryset=Inflow.objects.none(), prefix="inflow")
    inflow_form.helper.form_tag = False
    treatment_form = LogRemovalFormSet(queryset=LogRemoval.objects.none(), prefix="logremoval")
    treatment_form.helper.form_tag = False
    exposure_form = ExposureForm(prefix="exposure")
    exposure_form.helper.form_tag = False
    return render(request, "assessment-configurator.html",
                  dict(ra_form=ra_form,
